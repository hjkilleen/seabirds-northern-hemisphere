# Helper functions for analysis of global seabird breeding success
  ...
  #Modified by Helen Killeen (hkilleen@faralloninstitute.org)
...
# Dave's function to extract value from nearest coastal cell -------------------
# Function is called extractnearestcoast()
# It takes two arguements:
# 1 - The raster layer of values you're after
# 2 - Some points
extractnearestcoast <- function(r, xy, type = "land") { # Where r is a raster layer, xy are SpatialPoints, all projected lonlat, and type is either "land" (default) or "sea"
  if(type == "land") {
    coast <- boundaries(r, type = "inner")
  } else {
    coast <- boundaries(r, type = "outer")			
  ...
  ccells <- apply(XY, 1, FUN = function(x) {which.min(pointDistance(tsp@coords, x, lonlat = TRUE))})
  ccells <- unlist(ccells)
  return(raster::extract(r, tsp[ccells]))
}

		
#custom function to show data frame structure from https://gist.github.com/jbryer/4a0a5ab9fe7e1cf3be0e
